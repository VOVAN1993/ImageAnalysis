    ...
    if (dir) {
        ...
        while ((entry = readdir(dir)) != 0) {
            if (entry->d_type != 4) {
                ...
                vecL11.push_back(Image(compareByL1(nHistSample, nHist1), string(entry->d_name)));
                vecX22.push_back(Image(compareByX2(nHistSample, nHist1), string(entry->d_name)));
            }
        }
    }
